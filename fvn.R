...
#   dcls = a vector of classes corresponding to dvar
...
NN <- function(dvar, dcls, dfbk, dquery, radius, min.frac, min.cls, op) {
  ...
  d <- data.frame(dvar, class = dcls, positive = dfbk)
  ...
  u <- c() 
  for (icls in 1:ncls) {
    d_cls <- d %>% filter(class == lcls[icls])
    uvar <- c()
    for (ivar in 1:nvar) {
  ...
  for (iq in 1:nq) {
    ...
    sball <-
      ...
      data.frame()
    if (op == 'rate') {thesize = arrange(sball, neg_rate)$class[1]}
    if (op == 'popularity') {thesize = arrange(sball, desc(n_pos))$class[1]}
     drec <- c(drec, thesize)
  }
  return(drec)
}
